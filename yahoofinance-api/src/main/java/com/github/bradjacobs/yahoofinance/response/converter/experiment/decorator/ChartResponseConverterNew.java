...
public class ChartResponseConverterNew implements ResponseConverter
{
    ...
    private final ListToMapKeyDecorator listToMapKeyDecorator;


    public ChartResponseConverterNew() {
        this(null);
    }


    public ChartResponseConverterNew(ResponseConverterConfig config) {
        if (config == null) {
            config = ResponseConverterConfig.DEFAULT_INSTANCE; // if null, use instance w/ default values.
        }
        this.config = config;
        this.listToMapKeyDecorator = new ListToMapKeyDecorator(this, KEY_DATE);
    }

    @Override
    public Map<String, Map<String, Object>> convertToMapOfMaps(String json)
    {
        return listToMapKeyDecorator.convertToMapOfMaps(json);
    }
